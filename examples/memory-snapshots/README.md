...
`humemai:memory_id` becomes the reifier name (`humemai:m<id>`).

The RDF 1.2 files validate with Apache Jena `riot` (tested with Jena 6.1.0) and can
be queried with SPARQL 1.2 triple-term patterns via Jena `sparql`, e.g.
`?m rdf:reifies <<( ?s ?p ?o )>>`. Python tooling has not caught up yet: rdflib
does not parse RDF 1.2 syntax, so the reification files remain the loadable format
for the Python-based agents in this repository.
